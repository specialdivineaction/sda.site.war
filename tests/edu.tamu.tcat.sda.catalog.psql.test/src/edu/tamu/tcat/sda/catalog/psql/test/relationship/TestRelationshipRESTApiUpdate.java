...
public class TestRelationshipRESTApiUpdate
{
   ...
   public void testUpdateRelationship()
   {
      ...
      try
      {
         ...
         HttpResponse createResponse = client.execute(post);
         ...
         URI relationshipUri = uri.resolve("relationships/" + relnId.id);
         ...
         put.setURI(relationshipUri);
         put.setEntity(new StringEntity(mapper.asString(createdReln)));
         HttpResponse updateResponse = client.execute(put);
         int statusCode = updateResponse.getStatusLine().getStatusCode();
         if (statusCode < 200 && statusCode >= 300)
            fail("Response code:" + statusCode);

         CloseableHttpResponse getResponse2 = client.execute(get);
         try(InputStream is = getResponse2.getEntity().getContent())
         {
            updateReln = mapper.parse(is, RelationshipDV.class);
            Assert.assertEquals(relnDV.description, updateReln.description);
            Assert.assertNotEquals(relnDV.typeId, updateReln.typeId);
         }
